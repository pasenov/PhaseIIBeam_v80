...
void EventAction::EndOfEventAction(const G4Event* evt)
{
 ...
 for (G4int k = 0; k < 1017; k++) {
   ...
   if (k > 1)  {
      fChargeStripMod1a[k-1]+= (fChargeStrip1a[k])/2;
      fChargeStripMod1b[k-1]+= (fChargeStrip1b[k])/2;
      fChargeStripMod2a[k-1]+= (fChargeStrip2a[k])/2;
      fChargeStripMod2b[k-1]+= (fChargeStrip2b[k])/2;
   }
    ...
    for (G4int ke2 = 0; ke2 < 17; ke2++) {
 	for (G4int ke3 = 0; ke3 < 81; ke3++) {
 	   for (G4int ke4 = 0; ke4 < 53; ke4++) {
                ...
                if ((ke3 > 1))  {
		   fChargePixelMod[ke1][ke2][ke3-1][ke4] += fChargeQuad;
                }
                if ((ke3 < 80))  {
		   fChargePixelMod[ke1][ke2][ke3+1][ke4] += fChargeQuad;
                }
                if ((ke4 > 1))  {
		   fChargePixelMod[ke1][ke2][ke3][ke4-1] += fChargeQuad;
                }
                if ((ke4 < 52))  {
		   fChargePixelMod[ke1][ke2][ke3][ke4+1] += fChargeQuad;
                }
